...
class AnimalDataBloc extends Bloc<AnimalDataEvent, AnimalDataState> {
  ...
  Future<void> _fetchAnimalInfo(FetchAnimalDataEvent event,
      Emitter<AnimalDataState>emit) async {
    emit(state.copyWith(status: ApiCAllStatus.loading));
    await networkRequester.getRequest(
        "https://api.api-ninjas.com/v1/animals?name=${event.countryName}",
        "oV/w2XjPR8UO4gfF0MI+xQ==14Z3QRk1DaBUs5f9").then((value) {
      emit(state.copyWith(message: "Api call successfull",status: ApiCAllStatus.success,animalResponse: ));

        },).onError((error, stackTrace) {
      print("error  $error");
      print("line no $stackTrace");
      emit(state.copyWith(status: ApiCAllStatus.error,message: error.toString()));
